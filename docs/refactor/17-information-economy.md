...
| Tabla / entidad | Campos | DTO / consumidor | Clase | Fuente canónica y actualización | Riesgo / decisión |
| --- | --- | --- | --- | --- | --- |
| `ventas_stock` / `VentaStock` | `id`, `alumno_id`, `stock_id`, `cantidad`, `fecha`, `idempotency_key`, `version` | cargo/stock | A | `StockServicio.vender` | Hecho de venta. |
| `ventas_stock` / `VentaStock` | `precio_unitario` | cargo/recibo | B | Copiado del producto al vender | Snapshot histórico inmutable. |
| `ventas_stock` / `VentaStock` | `estado`, `reversal_idempotency_key` | reversión de venta | A | `StockServicio.revertirVenta` | Estado de la venta y clave de la única reversión. |
...
| `pagos` / `Pago` | `estado`, `reversal_idempotency_key`, `motivo_anulacion`, `fecha_anulacion` | detalle/lista | A | `PagoServicio.anularPago` | Historia mínima de anulación; constraint impide combinaciones inválidas. |
...
| `egresos` / `Egreso` | `idempotency_key`, `request_hash`, `estado`, `reversal_idempotency_key`, `motivo_anulacion`, `fecha_anulacion` | egreso list/detail | A | Registro/reversión de egreso | Hash distingue reintento de conflicto; estado conserva anulación. |
...
| `movimientos_credito` / `MovimientoCredito` | todos: `id`, `alumno_id`, `tipo`, `importe`, `pago_id`, `cargo_id`, `movimiento_revertido_id`, `usuario_id`, `idempotency_key`, `motivo`, `created_at` | saldo/movimiento de crédito | A | `PagoServicio` y `CreditoServicio`; append-only | Única autoridad del crédito. Saldo es C y no existe en `alumnos`. |
...
| `recibos_pendientes` / `ReciboPendiente` | `id`, `pago_id`, `tipo`, `estado`, `intentos`, `next_attempt_at`, `ultimo_error`, `created_at`, `processed_at` | sin payload frontend | A | Outbox específica y worker | Estado técnico único; no duplica contenido del recibo. |
