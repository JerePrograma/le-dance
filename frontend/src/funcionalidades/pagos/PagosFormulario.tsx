  ...
  const handleAgregarDetalle = useCallback(
    async (
      ...
      if (
        ...
        console.log("🔥 [DEBUG] Iniciando CUOTA sobre DISCIPLINA", {
          disciplina: values.disciplina,
          tarifa: values.tarifa,
          cantidadRaw: values.cantidad,
        });

        const disc = mappedDisciplinas.find(
          (d) => d.nombre === values.disciplina
        )!;
        console.log("🔍 [DEBUG] disc encontrado:", disc);
        ...
        console.log("💰 precioUnit:", precioUnit, "tarifaLabel:", tarifaLabel);

        const cantidad = Number(values.cantidad) || 1;
        const valorBase = precioUnit * cantidad; // 35000
        console.log("📊 cantidad:", cantidad, "valorBase:", valorBase);

        // descuento
        const insc = activeInscripciones.find(
          (i) => i.disciplina.id === disc.id
        )!;
        const pct = insc.bonificacion?.porcentajeDescuento ?? 0;
        const bonificacionId = insc.bonificacion?.id ?? null;
        console.log(
          "🔖 bonificación %:",
          pct,
          "bonificacionId:",
        ...
        console.log("➖ importeInicial tras descuento:", importeInicial);

        // recargo
        const fecha = obtenerFechaDePeriodo(values.periodoMensual);
        const { recargoAplicado, importeConRecargo } = calcularRecargoPorFecha(
          fecha,
          recargos,
          importeInicial
        );
        ...
        console.log("💵 importePendiente (aCobrar):", importeConRecargo);
        ...
        console.log("➕ nuevoDetalle a push:", nuevoDetalle);

        newDetails.push(nuevoDetalle);
        added = true;
        console.log("🎉 agregado correctamente. added =", added);
      }
