  ...
  const handleAgregarDetalle = useCallback(
    async (
      ...
      if (
        ...
        // 1) Armo la etiqueta completa (para descripción)
        const periodo = values.periodoMensual;
        const label =
          tarifaKey === "CUOTA"
            ? `${disc.nombre} - ${tarifaLabel} - ${periodo}`
        ...
        // … dentro de if (!added && tarifaKey === "CUOTA") …
        if (tarifaKey === "CUOTA") {
          // 1) Obtengo la inscripción y la bonificación si existe
          const inscripcion = activeInscripciones.find(
            (i) => i.disciplina.id === disc.id
          );
          const bonificacion = inscripcion?.bonificacion;

          // 2) Calculo el descuento y el importe inicial neto
          const importeDescuento = calcularDescuento(valorBase, bonificacion);
          const importeInicial = valorBase - importeDescuento;

          // 3) Convierto el periodo a fecha para el recargo
          const fecha = obtenerFechaDePeriodo(periodo);

          // 4) Aplico la lógica de recargo según fecha
          const { recargoAplicado, importeConRecargo } =
            calcularRecargoPorFecha(fecha, recargos, importeInicial);

          // 5) Extraigo el ID del recargo (o null)
          const recargoId = recargoAplicado?.id ?? null;

          // 6) Etiqueta corta para mostrar en 'cuotaOCantidad'
          const cuotaLabel = `${tarifaLabel} - ${periodo}`;

          // 7) Armo el payload para verificar en la API
          const payload: DetallePagoRegistroRequest = {
            id: 0,
            version: 0,
            alumno: values.alumno,
            descripcionConcepto: label, // p.ej. "Yoga - CUOTA - ABRIL DE 2025"
            cuotaOCantidad: cuotaLabel, // p.ej. "CUOTA - ABRIL DE 2025"
            valorBase,
            importeInicial,
            ...
            estadoPago: "",
            tipo: "", // se asigna más abajo
          };

          // 8) Verificación contra la API
          try {
          ...
          // 9) Finalmente lo agrego al array
          newDetails.push({
            ...payload,
            id: 0,
            tipo: "MENSUALIDAD",
