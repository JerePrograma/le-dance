...
const defaultValues: CobranzasFormValues = {
  ...
  autoRemoved: [],
  pagoParcial: 0,
  aplicarRecargo: true, // por defecto se aplica recargo
  tieneRecargo: true, // cada detalle se crea con true
};
...
const CobranzasForm: React.FC = () => {
  ...
  const handleAlumnoChange = useCallback(
    async (
      ...
      // Obtenemos los datos frescos del alumno
      let alumnoInfo;
      try {
        ...
        .then((pagoData) => {
          setInitialValues({
            ...
            detallePagos: pagoData.detallePagos.map((detalle: any) => ({
              ...
              tieneRecargo: detalle.tieneRecargo, // se respeta el valor original
              autoGenerated: true,
            })),
          ...
          if (descMayus.includes("MATRICULA")) {
            ...
            try {
              ...
              newDetails.push({
                ...
                autoGenerated: false,
                pagoId: null,
                // Se asigna según el estado global de aplicarRecargo
  ...
  // ----- onSubmit: se fuerza que cada detalle tenga "tieneRecargo" según aplicarRecargo
    ...
    try {
      ...
      const pagoRegistroRequest: PagoRegistroRequest = {
        ...
        detallePagos: detallePagos.map((d) => ({
          ...
          // Forzamos que tengaRecargo sea true salvo que se haya desactivado
    ...
    <ResponsiveContainer className="py-4">
      ...
      >
        {({ values, setFieldValue }) => {
          // Asignación por defecto del método de pago "EFECTIVO"
          useEffect(() => {
          ...
          // Actualiza totalCobrado en función de "aCobrar" de cada detalle
          useEffect(() => {
          ...
          // Al cambiar método de pago se reactiva el recargo y se reinician importes
          const selectedMetodoPago = metodosPago.find(
            (mp: MetodoPagoResponse) => mp.id === Number(values.metodoPagoId)
          );
          ...
          // Handler para quitar recargo: se desactiva y se reinician importes pendientes
