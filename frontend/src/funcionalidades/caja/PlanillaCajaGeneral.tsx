...
const PlanillaCajaGeneral: React.FC = () => {
  ...
  const totalEfectivo = lista.reduce(
  ...
  const handleVer = async () => {
    try {
      setLoading(true);
      const data: CajaDiariaDTO[] = await cajaApi.obtenerPlanillaGeneral(
        startDate,
        endDate
      );
      setLista(data);
  ...
  // Función helper para parsear una fecha en formato "YYYY-MM-DD" como fecha local
        ...
        {loading ? (
          ...
          <Tabla
            ...
            customRender={(item: CajaDiariaDTO) => [
              ...
              parseLocalDate(item.fecha).toLocaleDateString("es-AR"),
              item.rangoRecibos,
