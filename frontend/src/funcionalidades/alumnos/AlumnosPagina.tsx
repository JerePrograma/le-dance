...
const Alumnos = () => {
  ...
  // Reiniciamos la cantidad visible cuando cambian los filtros
  useEffect(() => {
    setVisibleCount(itemsPerLoad);
  }, [alumnosFiltradosYOrdenados, itemsPerLoad]);

  // Cortamos la lista de alumnos a mostrar según la paginación
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between gap-4">
            <div>
              ...
              <datalist id="nombres">
                {Array.from(
                  new Set(alumnos.map((a) => `${a.nombre} ${a.apellido}`))
                ).map((nombre) => (
