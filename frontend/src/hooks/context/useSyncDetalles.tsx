...
export const useSyncDetalles = (
  ...
  useEffect(() => {
    ...
    //    - Si no tienen id, se verifica si ya existe un manual que coincida en 'concepto' y 'valorBase'.
    const finalAutoDetails = autoDetailsNotRemoved.filter((autoDet) => {
      if (autoDet.id !== null) {
        return !manualIdentifiers.has(autoDet.id);
      } else {
        // Si id es null, se asume que es un detalle sin asignar; buscamos coincidencia en manuales.
