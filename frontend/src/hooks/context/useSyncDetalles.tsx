...
const mapDetallePagos = (
  ...
  console.log("[mapDetallePagos] Iniciando mapeo de detalles pendientes:", detallePendientes);
  const mapped = detallePendientes.map((det) => {
    ...
    if (existing) {
      console.log(`[mapDetallePagos] Se encontró detalle existente para id ${idKey}:`, existing);
    } else {
      console.log(`[mapDetallePagos] No se encontró detalle existente para id ${idKey}`);
    }
    ...
    const mappedDetail: DetallePagoRegistroRequestExt = {
      ...
      // Se prioriza el valorBase; si no existe, se toma otro campo (por ejemplo, montoOriginal) o 0.
...
export const useSyncDetalles = (alumnoData: AlumnoDataResponse | undefined) => {
  ...
  useEffect(() => {
    ...
    if (!alumnoData) {
      console.log("[useSyncDetalles] No hay alumnoData. Abortando sincronización.");
      return;
    }
    console.log("[useSyncDetalles] AlumnoData recibido:", alumnoData);
    console.log("[useSyncDetalles] Valores actuales del formulario (detallePagos):", values.detallePagos);
    console.log("[useSyncDetalles] Valores actuales del formulario (autoRemoved):", values.autoRemoved);
    ...
    console.log("[useSyncDetalles] Detalles automáticos mapeados:", autoDetails);

    // 2. Filtrar los detalles automáticos que hayan sido removidos manualmente y que tengan aCobrar distinto de 0.
    const removedIds = new Set(values.autoRemoved || []);
    console.log("[useSyncDetalles] IDs removidos manualmente:", Array.from(removedIds));
    const autoDetailsFiltered = autoDetails.filter((det) => {
      if (det.id != null) {
        ...
        console.log(
          ...
          } por removedIds (${removedIds.has(det.id)}) y aCobrar (${det.aCobrar})`
        );
        return include;
      }
      ...
      console.log(
        `[useSyncDetalles] Auto detalle sin id ${
          include ? "incluido" : "excluido"
        } por aCobrar (${det.aCobrar})`
      );
      return include;
    });
    console.log("[useSyncDetalles] Detalles automáticos filtrados:", autoDetailsFiltered);
    ...
    if (manualDetails.length > 0) {
      ...
      console.log("[useSyncDetalles] IDs de detalles manuales:", Array.from(manualIds));
      const finalAutoDetails = autoDetailsFiltered.filter((autoDet) => {
        if (autoDet.id != null) {
          const include = !manualIds.has(autoDet.id);
          if (!include) {
            console.log(`[useSyncDetalles] Auto detalle id=${autoDet.id} descartado porque ya existe manual.`);
          }
          return include;
        }
        ...
        if (exists) {
          console.log("[useSyncDetalles] Auto detalle sin id descartado por coincidencia de campos:", autoDet);
        }
        return !exists;
      });
      console.log("[useSyncDetalles] Detalles automáticos finales después de filtrar duplicados:", finalAutoDetails);
      mergedDetails = [...manualDetails, ...finalAutoDetails];
      console.log("[useSyncDetalles] Fusionando manuales y automáticos no duplicados:", mergedDetails);
    } else {
      mergedDetails = autoDetailsFiltered;
      console.log("[useSyncDetalles] No hay detalles manuales; usando automáticos:", mergedDetails);
    }
    ...
    if (!isEqual(mergedDetails, values.detallePagos)) {
      ...
      console.log("[useSyncDetalles] La lista final es igual a la actual. No se actualiza.");
    }
