...
const mapDetallePagos = (
  ...
  const mapped = detallePendientes.map((det) => {
    ...
    const mappedDetail: DetallePagoRegistroRequestExt = {
      autoGenerated: true,
      id: det.id, // Usamos el id original del DetallePago
      version: det.version,
      ...
      // Se agrega el campo 'tipo' para identificar matrícula o mensualidad
      tipo: det.tipo ? det.tipo.toUpperCase() : "",
    };
...
export const useSyncDetalles = (alumnoData: AlumnoDataResponse | undefined) => {
  ...
  useEffect(() => {
    ...
    // 2. Filtrar los detalles removidos manualmente, excepto aquellos de matrícula.
    const removedIds = new Set(values.autoRemoved || []);
    const autoDetailsFiltered = autoDetails.filter((det) => {
      // Forzar inclusión de detalles de matrícula siempre que tengan importe pendiente > 0.
      if (det.tipo === "MATRICULA") {
        return Number(det.importePendiente) > 0;
      }
      if (det.id != null) {
        const include =
