...
(
    ...
    observacion           TEXT, -- Observacion individual para el alumno en este mes
...
--------------------------------------------------------------------------------
-- Se generan registros para cada combinacion distinta de (asistencia_mensual_id, alumno_id)
...
SELECT DISTINCT
    -- Se asume que existe una relacion entre alumno y su inscripcion; si la tabla inscripciones
...
-- Ajusta el JOIN de inscripciones si tu modelo tiene otro identificador para la inscripcion.
...
-- 3.2 Agregar la nueva columna que referenciara a la tabla asistencias_alumno_mensual
ALTER TABLE asistencias_diarias
    ADD COLUMN asistencia_alumno_mensual_id BIGINT;

-- 3.3 Actualizar la nueva columna asignandole el id correcto de la tabla asistencias_alumno_mensual.
-- Se hace mediante un UPDATE que vincula la antigua combinacion (asistencia_mensual_id, alumno_id)
...
-- Revisa y ajusta el JOIN con la tabla inscripciones segun tu modelo.
...
-- 3.5 Agregar la nueva restriccion foranea en asistencias_diarias
ALTER TABLE asistencias_diarias
...
--------------------------------------------------------------------------------
-- Quitar la columna global "observacion" (ya que cada alumno tendra su propia observacion)
