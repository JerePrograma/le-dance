...
-- 2. Actualizar la columna alumno_id basandose en la relacion existente en la tabla pagos
UPDATE detalle_pagos dp
SET alumno_id = p.alumno_id
FROM pagos p
WHERE dp.pago_id = p.id;

-- Si la consulta anterior no retorna filas, se procede a establecer la restriccion NOT NULL:
ALTER TABLE detalle_pagos
    ALTER COLUMN alumno_id SET NOT NULL;

-- 3. Agregar la llave foranea para referenciar la tabla alumnos
ALTER TABLE detalle_pagos
